...
async def fetch_csv_metadata(model: str) -> Dict[str, Any]:
    """
    Fetch CSV header metadata from LLM Assistant API.
    Returns git commit, model version string, etc.
    """
    metadata = {
        "git_commit": "unknown",
        "model_version": model,
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            # Fetch model version and git commit from LLM Assistant
...
def create_annotation_csv(
    ...
    """
    Create CSV content from annotation results with metadata header.
    ...
    output.write(f"# Model: {model_display}\n")
...
async def annotate_csv(
    ...
    # Fetch metadata (git commit, model version) from LLM Assistant
    metadata = await fetch_csv_metadata(model)
    
    # Create output CSV with metadata header
    csv_content = create_annotation_csv(
        results,
        model=model,
