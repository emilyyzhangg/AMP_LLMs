...
| **v41** | **pending** | **Fix outcome Positive overcalling (3 fixes). Fix 2: Active guard — ACTIVE_NOT_RECRUITING + future/recent completion → deterministic Active. Removed Phase I auto-positive and Phase II/III >10yr backstop. Fix 3: Publication quality — `_classify_publication()` flags trial-specific vs general. Keyword scanning restricted to trial-specific literature only. Dossier shows [TRIAL-SPECIFIC]/[GENERAL] tags. Fix 1: Prompt rewrite — requires efficacy evidence for Positive (safety alone insufficient). `_POSITIVE_KW` split into `_EFFICACY_KW` + `_SAFETY_KW`. Post-LLM overrides and skip_verification use efficacy keywords only.** |
