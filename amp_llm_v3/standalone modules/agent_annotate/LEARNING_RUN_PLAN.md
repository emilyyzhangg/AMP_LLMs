...
| **82** | **v42.6.14 50-NCT delivery crash + approval narrowing** | **TBD** | **50** | **—/50** | **Pending fix merge** | **v42.6.14 (TBD)** | — | **Same 50-NCT set. Changes: init `not_specified_override=False` at function scope (fixes 10 delivery_mode crashes); narrow `_STRONG_EFFICACY` to require regulatory-qualified approval phrases ("FDA approved", "EMA approved", "regulatory approval", "received approval") — bare "approved" was matching review-article language like "EpiVacCorona was approved for emergency use". Expected: delivery recovers 73% → ~85%, Positive over-calls drop 4→3, outcome stays ~25% (remaining gap is GT/CT.gov divergence on COMPLETED+GT=active and UNKNOWN+GT=active trials — structural, not agent-fixable on this set).** |
