...
| **v28** | **planned** | **Pre-cascade _KNOWN_SEQUENCES check (fixes chicken-and-egg: peptide=False blocks sequence lookup → Rule 3 never fires). New sequences (CPT31, 68Ga-RM2, MB1707, thymic peptide). Replace phi4-mini:3.8b → llama3.1:8b (preserves Google/Alibaba/Meta diversity). Verifier evidence 30→15 citations. Fallback parser for unstructured verifier responses. Smart retry with 8 citations. Exclude parse-failed verifiers from consensus (None ≠ disagree). Broaden peptide definition "therapeutic" → "molecule" (vaccines, imaging, conjugates → True). "Trial says peptide" structured fact signal. Fix "empt" RfF truncation, sequence normalization, COVID keywords. Target: peptide 90%+, RfF 82%+.** |
