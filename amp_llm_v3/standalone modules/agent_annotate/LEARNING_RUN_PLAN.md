...
| 56 | v32 validation B | 9583e6660ebd | 50 | 30/50 | **Running** | v32 (458edbf) | — | Same commit as 55. |
| 57 | v32 prior run | db7d3f85e6f8 | 50 | 50/50 | **Complete** | v32 (2fb4750) | — | Pre-outcome-fix v32. Peptide 98%, delivery 79.5%, outcome 59.1%. |
| 58 | v33 smoke | TBD | 10 | —/10 | **Next** | v33 | — | 2 critical bugs (consensus AMP alias, delivery normalization) + outcome status injection + generic publication filter + RfF keyword expansion + glucagon sequence. |
...
| **v33** | **pending** | **2 critical bugs + 5 improvements: (1) consensus.py removed `"amp":"other"` alias — was blocking all non-deterministic AMP classifications since v24. (2) orchestrator.py fixed `_normalize_final_values` delivery mode — was outputting dead v23 categories (IV, Oral-Unspecified). (3) Outcome: structured status+hasResults injection from CT.gov metadata when Pass 1 says NOT FOUND. (4) Outcome: generic publication filter — trial-specificity gate prevents drug-class publications from blocking registry status heuristics. (5) Outcome: H3b backstop — Phase II/III >10yr completed without negative evidence → Positive. (6) RfF: expanded keyword coverage (dose-limiting toxicity, lack of efficacy, etc.). (7) Glucagon added to _KNOWN_SEQUENCES (29aa, fixes NCT03490942). Classification 8/8 disagreements confirmed as R1 annotation errors.** |
