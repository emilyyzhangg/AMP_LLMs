...
| **83** | **47-NCT outcome-clean slice** | **51a6c2a308f8** | **47** | **—/47** | **Running** | **v42.6.15 (f02d75da)** | — | **First real outcome baseline on a GT/registry-aligned set. Stratified from training CSV: 12 terminated + 9 failed + 13 positive + 13 unknown; excludes GT=active (the divergence bucket), prior-job NCTs, and fast_learning_batch_50. Picker script committed as scripts/pick_outcome_clean_slice.py (deterministic seed). Purpose: measure outcome accuracy without the GT/CT.gov divergence that capped the 50-NCT set at ~25%.** |
