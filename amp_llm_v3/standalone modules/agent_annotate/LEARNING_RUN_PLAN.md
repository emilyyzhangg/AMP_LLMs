...
| **79** | **v42.6.10 50-NCT fix validation** | **TBD** | **50** | **—/50** | **Pending fix merge** | **v42.6.10 (TBD)** | — | **Same 50-NCT set as #78. v42.6.10 changes: narrowed peptide=False cascade (sequence + classification only; delivery/outcome/RfF proceed normally), restored ANR Active guard (past-completion + not stale + no pubs + no hasResults → deterministic Active). Gate: delivery ≥80%, outcome ≥35%, RfF ≥60%, peptide ≥86%, classification ≥87% (no regression from #78). Predicted lift: delivery +25pp (cascade fix), outcome +25pp (5 cascaded + 3 ANR cases), RfF +60pp (cascade fix unlocks the legacy RfF agent to run on Terminated/Withdrawn trials).** |
