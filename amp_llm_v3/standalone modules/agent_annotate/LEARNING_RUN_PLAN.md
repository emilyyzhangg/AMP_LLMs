...
| **80** | **v42.6.11 50-NCT outcome over-call fix** | **TBD** | **50** | **—/50** | **Pending fix merge** | **v42.6.11 (TBD)** | — | **Same 50-NCT set as #78/#79. Changes: tightened DOSSIER_PROMPT rule 3 to require explicit primary-endpoint-met / p<0.05 / approval / phase-advancement signal; `_dossier_publication_override` requires ≥2 trial-specific pubs + strong efficacy keyword; `skip_verification` gate uses same strong-efficacy requirement; `_infer_from_dossier` fallback tightened. Gate: outcome ≥40% (from 21.4%), no regression on peptide/classification/delivery, RfF may lift as a downstream effect if outcome corrects from Positive-over-call to Terminated/Failed.** |
