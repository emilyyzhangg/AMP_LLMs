...
| **v41b** | **pending** | **Fix v41 overcorrection: (a) `_classify_publication()` default "general" → "trial_specific" (only explicit review signals tag as general). (b) Remove Active guard days_since<=180 block (only future completion forces Active). All other v41 changes preserved.** |
