...
| **78** | **v42.6.9 50-NCT recovery validation** | **c05f049fef32** | **50** | **—/50** | **Running** | **v42.6.9 (257810da)** | — | **All atomic-cut-over + efficiency-pack flags OFF (prefer_atomic_classification, prefer_atomic_failure_reason, skip_legacy_when_atomic, deterministic_peptide_pregate, skip_amp_research_for_non_peptides). Atomic still runs in shadow only. Same 50-NCT set as #75c/#76. Target: restore v40 baseline — peptide ≥90%, classification ≥90%, delivery ≥80%, outcome ≥60%, RfF ≥85%. Expected wall-time ~3-3.5h (no efficiency-pack speedup; per_drug_research_cache still UNWIRED into clients pending Phase 2). Validates that the regression is config-gated (recoverable) rather than baked into v42 code.** |
