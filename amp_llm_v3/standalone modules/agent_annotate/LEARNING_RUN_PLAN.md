...
| **81** | **v42.6.12 50-NCT status safety net validation** | **TBD** | **50** | **—/50** | **Pending fix merge** | **v42.6.12 (TBD)** | — | **Same 50-NCT set. Changes: prompt rule 1 rewritten — CT.gov ACTIVE_NOT_RECRUITING is default "Active, not recruiting" regardless of staleness (only "primary endpoint met" publication flips to Positive); post-LLM safety net maps Unknown → canonical status label for ANR / Recruiting / Not-yet-recruiting; `_infer_from_dossier` fallback returns Active for stale ANR. Gate: outcome ≥55% (expected ~64% from 11 Unknown-undercalls recovering), no Positive-overcall regression (≤3 still), delivery/classification/peptide no regression.** |
