...
| 55 | v32 smoke | TBD | 10 | —/10 | **Next** | v32 | — | Outcome fixes: regex boundary, terminated safety net, hasResults override. 10 failing NCTs from v31 validation. |
...
| **v32** | **pending** | **Outcome fixes: (1) Section boundary regex — ported _SECTION_BOUNDARY from failure_reason.py, \n[A-Z] never matched on lowered text, results_section captured everything causing false keyword matches. (2) Terminated safety net — Unknown + TERMINATED + no results posted → force Terminated. (3) hasResults override — Unknown + COMPLETED + results posted → force Positive. Delivery fixes: (4) Expanded oral keywords (tablet, capsule standalone). (5) Injection priority guard only fires on exactly 2 routes. Classification: 7 AMP→Other confirmed as definitional disagreements (not bugs).** |
