...
class PipelineOrchestrator:
    ...
    async def _resolve_drug_names(
        ...
        prompt = (
            ...
            f"STRICT RULES — these names drive automated database lookups, so a "
            f"wrong synonym sends research to the wrong drug:\n"
            f"- Only list a name if you are CERTAIN it is the EXACT SAME drug "
            f"(same active ingredient). NEVER list a different drug, and NEVER guess.\n"
            f"- If you are not certain of the generic name or have no confident "
            f"synonyms, use 'unknown' / 'none' — that is strongly preferred over a guess.\n"
            f"- Output ONLY the formatted lines below. Do NOT add notes, "
            f"explanations, parentheticals, disclaimers, chemical structures, or "
