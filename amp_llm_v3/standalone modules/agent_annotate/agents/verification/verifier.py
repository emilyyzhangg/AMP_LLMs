...
FIELD_PROMPTS = {
    ...
    "outcome": {
        "instruction": (
            ...
            "CRITICAL RULES:\n"
            "- Registry says TERMINATED → Check the evidence:\n"
            "  * Positive published results OR drug advanced to later phases/approval → 'Positive'\n"
            "  * Safety failure, futility, or lack of efficacy confirmed → 'Failed - completed trial'\n"
            "  * Business/operational termination with no efficacy signal → 'Terminated'\n"
            "  * No publications, no results, reason unclear → 'Terminated' (default)\n"
