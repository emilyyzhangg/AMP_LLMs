...
- **Validation slices:**
  ...
  - slice-H `76389352da32` (prod, commit `e4673808`, IN FLIGHT 2026-05-07): 4 slice-G failed-completed Unknown misses (direct Lever-3 test) + 12 pos→unk full-corpus misses (dominant outcome miss class) + 2 known-good positives (regression check) + 2 terminated (stability). Score with `python3 scripts/score_full_corpus.py --merged-json results/json/76389352da32.json`.
- **Workflow shift (user 2026-05-07):** code lands on main directly. Reason: dev autoupdater wipes local commits every 30s via `git reset --hard origin/dev`. Push requires `GIT_SSH_COMMAND="ssh -i ~/.ssh/id_ed25519 -o IdentitiesOnly=yes" git push origin main`. Jobs run on prod port 8005.
