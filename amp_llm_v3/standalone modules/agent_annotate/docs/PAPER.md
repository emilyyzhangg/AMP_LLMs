...
### 4.6b Legacy Test-Batch Cohort (n=50, re-scored on v42.11)

The 50-NCT `legacy_test_batch` is a subset of the original 680-NCT training CSV that was carved out as a held-out cohort *before* the formal train/val/test split was established (2026-05-11). It was used for the pre-formal-split production-gate certification (Job #101) on v42.7.22. v42.11 re-scored it via job `036fc5dea889` (2026-06-03, 4h38m wall, 5.57 min/trial, 50/50 successful), against `human_ground_truth_train_df.csv`:

| Field | Legacy_test_batch | Test (n=85) | Human IRA | Verdict |
|---|---|---|---|---|
| classification | 88.6% (39/44) | 97.1% | 92.4% | below IRA on this cohort |
| peptide | 97.9% (47/48) | 97.4% | 86.0% | beats IRA (+11.9 pp) |
| delivery_mode | 95.0% (38/40) | 88.2% | 88.8% | beats IRA |
| outcome | 88.4% (38/43) | 60.5% | 61.3% | substantially above the human ceiling — cohort effect |
| sequence | 38.5% (15/39) | 17.4% | 43.6% | matches val (38.3%) |
| RfF score-blind | 80.0% (20/25) | 100% | 92.3% | precision strong |
| RfF true recall | 66.7% (20/30) | 42.9% | n/a | far above test/val/dev |

The cohort was originally curated to be a clean, mostly-failed sample for early production-gate testing — its outcome distribution is heavily skewed toward Terminated / Withdrawn / Failed-completed, so the agent's deterministic terminal-status mappers and v42.8 strong-failure publication override fire on a much larger fraction of trials than they do on the broader cohorts. The 88.4% outcome accuracy is not a v42.11 capability claim — it is a function of cohort composition. The classification result (88.6%) is below IRA because the cohort over-samples therapeutic peptides with edge-case mechanism evidence (vaccine peptides, hormone analogues), exposing the few remaining classifier failure modes. Both observations validate rather than contradict the formal-cohort numbers: data composition dominates accuracy where the agent has reached its data-bound or subjectivity ceilings.

### 4.6c Master-Extension Cohort (n=994, no human ground truth)

A separate dataset-extension run covers the **994 NCTs from the annotator-master xlsx that have no human annotation** (or have partial human annotation but were never curated into a formal cohort). Job `c74ce600868d` (2026-06-05, 2h41m wall, 9.7 s/trial, 994/994 successful) annotates every such trial. The dramatically lower per-trial wall time (compared to 5.57–6.29 min on the GT cohorts) reflects two cohort effects: (i) the AMP-relevant subset over-represents trials with deterministic-resolvable interventions (95.8% of trials hit `Peptide=True`, and the v42.10 peptide-anchor settles a large fraction at 96.6% precision without invoking the LLM); (ii) trials with `peptide=False` cascade most downstream fields to `N/A` without any LLM call.

Because no human ground truth exists for these 994 trials, no IRA scoring is possible — the output is pure agent annotation, intended for downstream use as a dataset extension. The 994 trials bring the agent's total annotation coverage to **1,844 NCTs** (629 train + 86 val + 85 test + 50 legacy + 994 master extension), spanning the entire AMP-relevant clinical-trial universe currently curated by the annotation team.
...
5. **Public dataset release** — **landed.** The full v42.11 annotation corpus is released at `Final Agent Annotations/` in the repository: per-cohort CSVs (train 629, val 86, test 85, legacy 50, master-extension 994 = **1,844 unique NCTs**) plus three consolidated single-file views (`ALL_consolidated__train_val_test__800_NCTs.csv`, `…__with_legacy_test_batch__850_NCTs.csv`, `…__full_universe__1844_NCTs.csv`). Each row carries 30 columns (NCT ID + 5 study-metadata columns + 6 fields × 4 columns each: value, evidence identifiers, sources, evidence text); consolidated views prepend `Cohort` and `Source Job ID`. Companion `README.md` documents cohort definitions, per-cohort accuracy vs human IRA, the LLM stack, and reproduction instructions.
