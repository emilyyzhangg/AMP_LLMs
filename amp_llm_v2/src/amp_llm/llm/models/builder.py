...
# ------------------------------------------------------------
# 🔒 Silent SSH runner — prevents input echo and disables TTY
async def _run_silent(ssh_connection, command: str, check: bool = False):
    """
    Run remote SSH command silently (no echo, no TTY allocation).
    """
    wrapped = f'bash -l -c "stty -echo; {command}; stty echo"'
    result = await ssh_connection.run(wrapped, check=check, term_type=None)
    return result
...
async def build_custom_model(
    ...
    try:
        ...
        result = await _run_silent(
            ssh_connection,
            f'ollama create {model_name} -f {temp_path}',
            check=False
        )
...
def _update_modelfile_base(modelfile_content: str, base_model: str) -> str:
    """Replace FROM line in Modelfile."""
    import re
    updated = re.sub(
        r'^FROM\s+\S+',
        f'FROM {base_model}',
