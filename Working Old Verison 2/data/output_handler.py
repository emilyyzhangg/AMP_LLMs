...
def flatten_pubmed_studies(pubmed_section):
    """Flatten PubMed 'studies' list into rows for export."""
...
def flatten_pmc_summaries(pmc_section):
    """Flatten PMC 'summaries' list into rows for export."""
    if not pmc_section:
        return []
    rows = []
    for item in pmc_section.get("summaries", []):
        ...
        # metadata is a map keyed by uid -> meta
...
def flatten_clinical_trial(clinical_section):
    """Flatten a small set of clinical trial fields for overview sheet."""
    if not clinical_section:
        return []
    data = clinical_section.get("data", {}) or {}
    # try to pull common fields from the 'v2' clinicaltrials JSON shape
...
def save_results_to_csv(result, filename):
    """Main CSV exporter (flatten PubMed + PMC rows into a single CSV)."""
    sources = result.get("sources", {})
    pubmed_rows = flatten_pubmed_studies(sources.get("pubmed"))
    pmc_rows = flatten_pmc_summaries(sources.get("pmc"))

    # If both present, concatenate; else fallback to whichever exists
    rows = []
    if pubmed_rows:
...
def save_results_to_excel(result, filename):
    """Write multi-sheet Excel: ClinicalTrials / PubMed / PMC / Raw_JSON"""
    sources = result.get("sources", {})
    pubmed_rows = flatten_pubmed_studies(sources.get("pubmed"))
    pmc_rows = flatten_pmc_summaries(sources.get("pmc"))
...
def save_responses_to_excel(conversation, filename, csv_mode=False):
    """Save an interactive conversation list (list of dicts) to CSV or XLSX."""
    if not conversation:
...
def save_any(obj, filename, filetype="xlsx"):
    """Convenience dispatcher for saving either a conversation (list) or result (dict)."""
