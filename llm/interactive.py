...
def interactive_session(ssh_client=None, model_name=None, study_info=None):
    ...
    conversation = []
    while True:
        user_input = input("\nEnter PMID, NCT ID, or prompt: ").strip()
        ...
        if user_input.isdigit():
            ...
            source = "pubmed"
            conversation.append({"prompt": f"PubMed study {pmid}", "response": json.dumps(study_info, indent=2), "source": "pubmed_api"})
            ...
            if "error" in trial_info:
                ...
                continue
            pretty_print(trial_info, f"ClinicalTrials.gov + PubMed + PMC for {nct_id}")
            conversation.append({"prompt": f"NCT ID: {nct_id}", "response": json.dumps(trial_info, indent=2), "source": "clinicaltrials_api"})
            pubmed_studies = trial_info.get("sources", {}).get("pubmed", {}).get("studies", [])
            for study in pubmed_studies:
                pmid = study.get("pmid", "Unknown PMID")
                pretty_print(study, f"PubMed Study for PMID {pmid}")
                conversation.append({"prompt": f"PubMed study for PMID {pmid} (related to {nct_id})", "response": json.dumps(study, indent=2), "source": "pubmed_api"})
            combined_data = {"clinical_trial": trial_info.get("sources", {}).get("clinical_trials", {}), "pubmed": trial_info.get("sources", {}).get("pubmed", {}), "pmc": trial_info.get("sources", {}).get("pmc", {})}
            prompt = f"Summarize this clinical trial and its related PubMed/PMC studies:\\n\\n{json.dumps(combined_data, indent=2)}"
        ...
        if prompt:
            try:
                ...
                print("\\n🧠 LLM Response:\\n", response)
                conversation.append({"prompt": user_input, "response": response, "source": source})
